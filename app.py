...
# 🔴 ضع رقمك الشخصي هنا (الذي تريد من صديقك أن يرسل إليه) بمفتاحه الدولي وبدون أصفار أو +
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
        ...
        .sidebar { width: 420px; min-width: 420px; background: #fff; display: flex; flex-direction: column; border-left: 1px solid #e9edef; }
        .sidebar-header { height: 60px; background: #f0f2f5; display: flex; align-items: center; padding: 0 10px; justify-content: space-between; direction: rtl; }
        .header-right-side { display: flex; align-items: center; gap: 5px; }
        .avatar { width: 35px; height: 35px; background: #dfe5e7; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #54656f; font-weight: bold; font-size: 12px; }
        .header-actions { display: flex; align-items: center; gap: 5px; margin-right: auto; }
        
        .text-btn { border: none; padding: 6px 10px; border-radius: 6px; cursor: pointer; font-size: 12px; font-weight: bold; color: white; display: inline-flex; align-items: center; justify-content: center; transition: opacity 0.2s; text-decoration: none; }
        .text-btn:hover { opacity: 0.9; }
        .text-btn.broadcast { background-color: #0284c7; }
        .text-btn.new-chat { background-color: #00a884; }
        .text-btn.contact-owner { background-color: #ea580c; } /* لون برتقالي لزر مراسلتك */
...
<body>
    ...
    <div class="sidebar">
        <div class="sidebar-header">
            ...
            <!-- الأزرار الثلاثة بما فيها زر مراسلتك المباشر -->
            <div class="header-actions">
                <a href="https://wa.me/{{ personal_phone }}?text=مرحباً%20يا%20صديقي،%20أنا%20أستخدم%20موقعك%20الآن!" target="_blank" class="text-btn contact-owner">📱 راسل المالك</a>
                <button class="text-btn broadcast" onclick="openBroadcastModal()">📢 بث للكل</button>
                <button class="text-btn new-chat" onclick="openNewChatModal()">➕ رقم</button>
            </div>
        </div>
        <div class="search-box"><div class="search-inner"><input type="text" id="searchInput" placeholder="البحث عن دردشة..." oninput="filterUsers()"></div></div>
    ...
    <div class="welcome-screen">
        ...
        <p>استخدم الأزرار في القائمة الجانبية لبدء التحكم والعمل المباشر.</p>
    </div>
    ...
    <script>
        ...
        function sendBroadcast() {
            ...
            fetch('/api/broadcast', { method: 'POST', body: formData }).then(res => res.json()).then(data => {
                if(data.status === 'ok') { alert(`تم إرسال البرودكاست بنجاح 🚀`); closeBroadcastModal(); fetchUsersList(); }
            });
        }
        ...
        function createNewChat() {
            ...
            fetch('/api/add_user', { method: 'POST', body: formData }).then(res => res.json()).then(data => {
                if(data.status === 'ok') { closeNewChatModal(); fetchUsersList(); openChat(phone, null); }
            });
        }
        ...
        function fetchMessages() {
            ...
            fetch(`/messages/${currentPhone}`).then(res => res.json()).then(data => {
                ...
                data.messages.forEach(msg => {
                    ...
                    row.innerHTML = `<div class="bubble"><span>${msg.message}</span><div class="meta-container"><span class="time-text">${msg.msg_time || ''}</span></div></div>`;
        ...
        function sendMessage(event) {
            ...
            formData.append('phone', phone);
            formData.append('message', message);
            fetch('/send', { method: 'POST', body: formData }).then(res => res.json()).then(data => { input.value = ""; fetchMessages(); });
        }
    </script>
</body>
</html>
...
@app.route("/chat")
def chat_panel():
    return render_template_string(HTML_TEMPLATE, personal_phone=MY_PERSONAL_PHONE)
...
def broadcast():
    ...
    for user in users:
        ...
        c.execute("INSERT OR IGNORE INTO messages (msg_id, phone, message, sender, status, msg_time) VALUES (?, ?, ?, 'me', 'sent', ?)", (wamid, phone, message, now_time))
        sent_count += 1
    conn.commit()
    conn.close()
...
def send():
    ...
    c = conn.cursor()
    c.execute("INSERT OR IGNORE INTO users VALUES (?)", (phone,))
    c.execute("INSERT INTO messages (msg_id, phone, message, sender, status, msg_time) VALUES (?, ?, ?, 'me', 'sent', ?)", (wamid, phone, message, now_time))
    conn.commit()
    conn.close()
    return jsonify({"status": "ok"})
...
def webhook():
    ...
    try:
        ...
        if "messages" in value:
            ...
            # إرسال إشعار تلقائي لك أيضاً على واتساب
            notification_text = f"🔔 رسالة جديدة من: {phone}\nالرسالة: {text}"
